...
Suppose $\gamma = h(\theta)$ for some injective function $h$. As an example, suppose we impose a non-informative prior on $\Theta$ for $\theta > 0$, and that $h = \log$. We have that $\theta = e^{\gamma}$ and
\begin{equation}
...
\end{align}
It turns out there is such a function, and for this reason, we say that $g$ is **invariant**. 

Suppose, from Lehmann and Casella Lemma 5.3, we assume the following conditions hold:
...
\begin{equation}
I(\tau) = -\mathbb{E}_{\mathbf{X} \mid T}\left[\dfrac{\mathrm{d}^2}{\mathrm{d}\tau^2}\log f_{\mathbf{X} \mid T}(\mathbf{X} \mid \tau)\right]\text{.}
\end{equation}
